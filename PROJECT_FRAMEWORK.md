...
│
├── configs/                 ← 实验配置（YAML）
│   ├── m2_centralized_full_2.8b.yaml    ← 新 M2 主锚点
│   ├── m2_centralized_full_1.4b.yaml    ← 新 M2 辅助锚点
│   ├── m3_fedavg_full_2.8b.yaml         ← 新 M3 主实验
│   ├── m3_fedavg_full_1.4b.yaml         ← 新 M3 辅助（规模扫描）
│   └── m3_fedavg_head_llama_8b.yaml     ← 跨架构验证
│
├── src/fclprm/
│   ├── models/base_wrapper.py    ← StepRewardModel（freeze_backbone 可选）
│   ├── models/prm_head.py        ← PRMHead
│   ├── metrics/cd_spi.py         ← 跨客户端 divergence 度量
│   ├── federated/
...
| 假设 | 验证实验 | 如不成立的影响 |
|------|---------|--------------|
| H1: 全参数 FT 在 centralized 上显著优于 head-only | M2-full vs 旧 M2 (99.7%) | 核心假设错误 |
